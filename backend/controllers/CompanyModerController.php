...
use common\models\CompanyModerSearch;
 ...
 */
class CompanyModerController extends Controller
{
    /**
     * {@inheritdoc}
     */
    ...
    public function actionIndex()
    {
        ...
        $dataProviderWaiting->query->where(['status' => Company::STATUS_WAITING])->orderBy('updated_at DESC');

        $dataProviderActive = $searchModel->search(Yii::$app->request->queryParams);
        $dataProviderActive->query->where(['status' => Company::STATUS_ACTIVE])->orderBy('updated_at DESC');

        $dataProviderDeleted = $searchModel->search(Yii::$app->request->queryParams);
        $dataProviderDeleted->query->where(['status' => Company::STATUS_DEACTIVE])->orderBy('updated_at DESC');
    ...
    {
        if (!$model = Company::findOne($id)) return $this->redirect(['/company-moder']);
        $question_moder = CompanyModer::find()->where(['company_id' => $model->id])->one();
        ...
        if ($model->save(false)) {
            ...
            return $this->redirect(['/company-moder']);
        } else {
    ...
    public function actionDeactive($id)
    {
        ...
        if (!$product) return $this->redirect('/company-moder');

        $model = new CompanyListFrom();
        ...
        if ($model->load(Yii::$app->request->post())) {
            ...
            $product->status = Company::STATUS_DEACTIVE;

            if (!$product->save(false)) {
                throw new ErrorException('No De active product');
            }

            $Pmoder = new CompanyModer();

            $Pmoder->reason_id = $model->reason_id;
            $Pmoder->company_id = $model->company_id;
            $Pmoder->created = time();
