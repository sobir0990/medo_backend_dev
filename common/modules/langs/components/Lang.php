<?php
namespace common\modules\langs\components;
...
class Lang extends Component{
    ...
    * это значение сдесь
    */
    const LANG_HASH_GET_NAME = 'langHash';
    /*
    ...
    public static function getLangId($code = null){
        if($code == null){
            return Langs::find()->code(Yii::$app->language)->one()->lang_id;
        }else{
            return Langs::find()->code($code)->one()->lang_id;
        }
    }
    /*
    ...
    public static function withoutTranslate(ActiveRecord $model,$withoutlangs = true,$index_by = 'primaryKey'){
        ...
        $translates = $model::find()->select(['lang','lang_hash', $model::tableName().'.'.$model::primaryKey()[0]])->where(['lang_hash' => $model->lang_hash])->indexBy($index_by_t)->asArray()->all();
