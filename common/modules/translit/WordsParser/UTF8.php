...
class UTF8
{
	...
	public static $html_special_chars_table = [
        '&quot;' => "\x22",
        '&amp;'  => "\x26",
        '&lt;'   => "\x3c",
        '&gt;'   => "\x3e",
	...
	public static $html_entity_table = [
	...
	public static $convert_case_table = [
        "\x41" => "\x61", #A a
        "\x42" => "\x62", #B b
        "\x43" => "\x63", #C c
        "\x44" => "\x64", #D d
	...
	private static function _convert($data, $charset_from, $charset_to)
	{
		...
		if (is_string($data)) {
			if ($charset_from === 'UTF-8' && ! self::is_utf8($data)) {
                return $data;
            }

			if ($charset_to === 'UTF-8' && self::is_utf8($data)) {
                return $data;
            }

			if (function_exists('iconv')) {
                return iconv($charset_from, $charset_to . '//IGNORE//TRANSLIT', $data);
            }

			if (function_exists('mb_convert_encoding')) {
                return mb_convert_encoding($data, $charset_to, $charset_from);
            }

			if ($charset_from === 'UTF-16' || $charset_from === 'UCS-2') {
                return self::_convert_from_utf16($data);
            }

			if ($charset_from === 'cp1251' || $charset_from === 'cp1259') {
                return strtr($data, self::$cp1259_table);
            }

			if ($charset_from === 'koi8-r' || $charset_from === 'KOI8-R') {
                return strtr(convert_cyr_string($data, 'k', 'w'), self::$cp1259_table);
            }

			if ($charset_from === 'iso8859-5') {
                return strtr(convert_cyr_string($data, 'i', 'w'), self::$cp1259_table);
            }

			if ($charset_from === 'cp866') {
                return strtr(convert_cyr_string($data, 'a', 'w'), self::$cp1259_table);
            }

			if ($charset_from === 'mac-cyrillic') {
                return strtr(convert_cyr_string($data, 'm', 'w'), self::$cp1259_table);
            }
	...
	private static function _convert_from_utf16($s, $type = 'BE', $to_array = false)
	{
		...
		if (function_exists('iconv') || function_exists('mb_convert_encoding')) {
			if (function_exists('iconv')) {
                $s = iconv('UTF-16' . $type, 'UTF-8', $s);
            } elseif (function_exists('mb_convert_encoding')) {
                $s = mb_convert_encoding($s, 'UTF-8', 'UTF-16' . $type);
            }
		...
		foreach (unpack($types[$type] . '*', $s) as $codepoint) {
			if ($hi !== false) {
				...
				$hi = false;
			} elseif ($codepoint < 0xD800 || $codepoint > 0xDBFF) {
                $a[] = self::chr($codepoint);
            } else {
                $hi = $codepoint;
            }
	...
	public static function is_utf8($data, $is_strict = true)
	{
		...
		if (is_string($data)) {
			...
			if ($is_strict && preg_match('/[^\x09\x0A\x0D\x20-\xBF\xC2-\xF7]/sSX', $data)) {
                return false;
            }
			...
			foreach ($chars as $i => $char) {
				...
				foreach ($blocks as $j => $block) {
					if (is_string($block)) {
                        ...
                        trigger_error('A string/array/int type expected for block[' . $j . ']!', E_USER_ERROR);
                    }
	...
	public static function to_unicode($s)
	{
		...
		$s2 = null;
		if (function_exists('iconv')) {
            $s2 = @iconv('UTF-8', 'UCS-4BE', $s);
        } elseif (function_exists('mb_convert_encoding')) {
            $s2 = @mb_convert_encoding($s, 'UCS-4BE', 'UTF-8');
        }

        if (is_string($s2)) {
            return array_values(unpack('N*', $s2));
        }
	...
	public static function ord($char)
	{
		...
		static $cache = [];
		if (array_key_exists($char, $cache)) {
            return $cache[$char];
        }
	...
	public static function chr($cp)
	{
		...
		if ($cp <= 0x7f) {
            return $cache[$cp] = chr($cp);
        }

		if ($cp <= 0x7ff) {
            return $cache[$cp] = chr(0xc0 | ($cp >> 6))  .
                chr(0x80 | ($cp & 0x3f));
        }

		if ($cp <= 0xffff) {
            return $cache[$cp] = chr(0xe0 | ($cp >> 12)) .
                chr(0x80 | (($cp >> 6) & 0x3f)) .
                chr(0x80 | ($cp & 0x3f));
        }

		if ($cp <= 0x10ffff) {
            return $cache[$cp] = chr(0xf0 | ($cp >> 18)) .
                chr(0x80 | (($cp >> 12) & 0x3f)) .
                chr(0x80 | (($cp >> 6) & 0x3f)) .
                chr(0x80 | ($cp & 0x3f));
        }
	...
	public static function chunk_split($s, $length = null, $glue = null)
	{
		...
		if (!is_array($a = self::str_split($s, $length))) {
            return false;
        }
	...
	public static function convert_case($data, $mode, $is_ascii_optimization = true)
	{
		...
		if ($mode === CASE_UPPER) {
			if ($is_ascii_optimization && self::is_ascii($data)) {
		...
		if (strpos($s, '&#') !== false) {
            ...
            function (array $m) use ($class, $html_special_chars_table_flipped, $is_special_chars) {
                ...
                if (!$is_special_chars) {
                    ...
                    if (array_key_exists($char, $html_special_chars_table_flipped)) {
                        return $html_special_chars_table_flipped[$char];
                    }
                }
		...
		if (0 && preg_match_all('~(?>	[\xc2\xc3\xc5\xc6\xcb\xce\xcf][\x80-\xbf]  #2 bytes
			...
			foreach (array_unique($m[0]) as $char) {
				if (array_key_exists($char, $table)) {
                    $s = str_replace($char, $table[$char], $s);
                }
			}
		}
	...
	public static function str_limit($s, $maxlength = null, $continue = "\xe2\x80\xa6", &$is_cutted = null, $tail_min_length = 20) #"\xe2\x80\xa6" = "&hellip;"
	{
		...
		$s2 = preg_replace('/&(?> [a-zA-Z][a-zA-Z\d]+
                            ...
                            /sxSX', '?', $s2);
		if (strlen($s2) <= $maxlength || self::strlen($s2) <= $maxlength) {
            return $s;
        }
		...
		if (self::strlen($s) - self::strlen($return) < $tail_min_length) {
            return $s;
        }
	...
	public static function strpos($s, $needle, $offset = null)
	{
		...
		$byte_pos = $offset;
		do if (($byte_pos = strpos($s, $needle, $byte_pos)) === false) {
            return false;
        }
	...
	public static function stripos($s, $needle, $offset = null)
	{
		...
		if (function_exists('mb_stripos')) {
            return mb_stripos($s, $needle, $offset, 'utf-8');
        }
	...
	public static function strrev($s)
	{
		...
		if (!is_array($a = self::str_split($s))) {
            return false;
        }
	...
	public static function unescape_request()
	{
		...
		foreach (['_GET' => isset($_SERVER['QUERY_STRING']) ? $_SERVER['QUERY_STRING'] : null,
	...
	private static function _preg_quote_class($charlist, $delimiter = null)
	{
		...
		if (is_string($delimiter)) {
            $quote_table[$delimiter] = '\\' . $delimiter;
        }
	...
	public static function strspn($str, $mask, $start = null, $length = null)
	{
		...
		if (preg_match('~^[' . preg_quote($mask, '~') . ']+~uSX', $str, $m)) {
            self::strlen($m[0]);
        }
		...
		$is_simulate = false)
	{
		...
		while (($name = readdir($dh)) !== false) {
			...
			$file = $dir . '/' . $name;
			if (is_file($file)) {
				if (is_string($files_re) && ! preg_match($files_re, $name)) {
                    continue;
                }
