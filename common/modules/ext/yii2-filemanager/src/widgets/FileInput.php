...
JS;
        ...
        if ($this->static) {
            $js = <<<JS
 var func = function () {
        document.fileManager = new FileManager("#{$this->id}-sortable","#{$this->id}","{$this->delimtr}","{$this->getFileItemCloseBtn()}");
        $('#{$this->getModalId()}') . modal('show');
    };
        $('#{$this->getAddBtnId()}').click(func);
...
JS;
        ...
        $filemanagerJsObject = $this->getFileManagerModalId();
        Yii::$app->view->registerJs($js);
        ...
        return ModalWidget::widget([
            'fileManagerJsObject' => $filemanagerJsObject,
            'modal_options'       => [
                'id' => $this->id . "_modal",
            ],
