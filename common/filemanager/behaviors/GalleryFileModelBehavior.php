...
class GalleryFileModelBehavior extends AttributeBehavior
{
    /**
    ...
    private function linkData(){
        ...
        if($data):
            $titles = \Yii::$app->request->post('FileTitle');
            $descriptions = \Yii::$app->request->post('FileDescription');
            $i = 0;
            foreach ($data as $d)
            {
                $element = $elements[$d];
                ...
                Yii::$app->db->createCommand()->update($this->via_table_name,['sort' => $i, 'title' => $titles[$d], 'description' => $descriptions[$d]],$sql)->execute();
            }
        endif;
    }
